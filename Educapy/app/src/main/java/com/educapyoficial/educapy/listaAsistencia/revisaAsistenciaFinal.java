...
public class revisaAsistenciaFinal extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDialog = new SpotsDialog.Builder().setContext(revisaAsistenciaFinal.this).setMessage("Espere Un Momento").build();
    ...
    private void listarDatos() {
        databaseReference.child("Asistencia").child(jaloFecha).addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull final DataSnapshot dataSnapshot) {
                ...
                Toast.makeText(revisaAsistenciaFinal.this, jaloFecha.toString(), Toast.LENGTH_SHORT).show();
                for (DataSnapshot objSnaptshot : dataSnapshot.getChildren()) {
                    int count = 0;
                    maxid = (dataSnapshot.getChildrenCount());
                    constructoAsistencia p = objSnaptshot.getValue(constructoAsistencia.class);
                    listEspecialidad.add(p);
                    arrayAdapterEspecialidad = new ArrayAdapter<constructoAsistencia>(revisaAsistenciaFinal.this, android.R.layout.simple_list_item_1, listEspecialidad);
