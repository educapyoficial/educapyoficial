...
public class revisaAsistenciaFinalAlumno extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDialog = new SpotsDialog.Builder().setContext(revisaAsistenciaFinalAlumno.this).setMessage("Espere Un Momento").build();
    ...
    private void listarDatos() {
        databaseReference.child("Asistencia").child(jaloFecha).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull final DataSnapshot dataSnapshot) {
                ...
                Toast.makeText(revisaAsistenciaFinalAlumno.this, jaloFecha.toString(), Toast.LENGTH_SHORT).show();
                for (DataSnapshot objSnaptshot : dataSnapshot.getChildren()) {
                    int count = 0;
                    maxid = (dataSnapshot.getChildrenCount());
                    constructoAsistencia p = objSnaptshot.getValue(constructoAsistencia.class);
                    listEspecialidad.add(p);
                    arrayAdapterEspecialidad = new ArrayAdapter<constructoAsistencia>(revisaAsistenciaFinalAlumno.this, android.R.layout.simple_list_item_1, listEspecialidad);
